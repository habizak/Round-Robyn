...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Updated: get elements using the new IDs
    const winningPointsSelect = document.getElementById('winning-points-select');
    ...
    function generateTournament() {
        ...
        // Check if the number of courts is feasible based on the number of players and match type
        if (matchType === 'singles') {
            const maxCourts = Math.floor(numPlayers / 2);
            if (courtCount > maxCourts) {
                alert(`Too many courts! With ${numPlayers} players, you can only use up to ${maxCourts} court(s) concurrently for singles matches.`);
                return;
            }
        } else if (matchType === 'fixed-doubles' || matchType === 'random-doubles') {
            // For doubles, players are paired into teams
            if (numPlayers % 2 !== 0) {
                alert('Doubles match types require an even number of players');
                return;
            }
            const numTeams = numPlayers / 2;
            const maxCourts = Math.floor(numTeams / 2);
            if (courtCount > maxCourts) {
                alert(`Too many courts! With ${numPlayers} players (i.e. ${numTeams} teams), you can only use up to ${maxCourts} court(s) concurrently for doubles matches.`);
                return;
            }
        }
    ...
    function validateStep(step) {
        switch(step) {
            case 2: // Player Names step
                ...
                // Feasibility check for courts based on players for singles (default)
                const maxCourtsSingles = Math.floor(players.length / 2);
                if (courts.length > maxCourtsSingles) {
                    alert(`Too many courts! With ${players.length} players, you can only have up to ${maxCourtsSingles} court(s) concurrently for singles matches.`);
                    return false;
                }
                return true;
